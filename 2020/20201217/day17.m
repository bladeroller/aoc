...
matrix = zeros(nrows, ncols, nslices);

% set the initial state
matrix(num_cycles+1:num_cycles + num_initial_rows, num_cycles+1:num_cycles+num_initial_cols, num_cycles + 1) = initial
...
for n = 1:num_cycles
  ...
  for z = 1:nslices
    ...
    if all(all(all( snapshot(:,:,z_to_check) == 0 )))
    ...
    for r = 1:nrows
      for c = 1:ncols
        cube = snapshot(r, c, z);
        snapshot(r, c, z) = 255;  % mark the original cube
        ...
        neighbors = snapshot(row_to_check, col_to_check, z_to_check);
        
        num_active_neighbors = sum(sum(sum( neighbors == 1 )));
        if cube == 1
          ...
          if num_active_neighbors == 3,  matrix(r, c, z) = 1;
          else                        ,  matrix(r, c, z) = 0;
          end
        end
        snapshot(r, c, z) = cube;  % reset the origial cube
      end
    end
